...
public class GameManager {
    ...
    private game.Player whitePlayer;
    private game.Player blackPlayer;
    private game.Player activePlayer;
    private game.Chessman selectedChessman;
    private ArrayList<game.Move> allGameMoves;
    private boolean moveDone;
    private ArrayList<game.Chessman> killedChessmen;
    private int limit;
    private boolean undoUsed;

    public GameManager(game.User whiteUser, game.User blackUser, int limit) {
        board = new game.BoardUnitSquare[8][8];
        killedChessmen = new ArrayList<>();
        allGameMoves = new ArrayList<>();
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[i].length; j++) {
                board[i][j] = new game.BoardUnitSquare(i, j, board);
            }
        }
        this.whitePlayer = new game.Player(whiteUser, 'w');
        this.blackPlayer = new game.Player(blackUser, 'b');
    ...
    public String processChessmanSelecting(int posX, int posY) {
        game.BoardUnitSquare inputSquare = getSquareByPosition(posX, posY);
    ...
    public game.BoardUnitSquare getSquareByPosition(int posX, int posY) {
    ...
    public String processMove(int destinationPosX, int destinationPosY) {
        ...
        game.BoardUnitSquare destinationSquare = getSquareByPosition(destinationPosX, destinationPosY);
        if (selectedChessman.isMovePossible(selectedChessman.getCurrentPosition(), destinationSquare)) {
            setMoveDone(true);
            game.Move lastMove = new game.Move(selectedChessman.getCurrentPosition(), destinationSquare, selectedChessman, destinationSquare.getCurrentChessman(), activePlayer);
    ...
    public String processUndo() {
        ...
        undoUsed = true;
        moveDone = false;
        game.Move lastMove = allGameMoves.get(allGameMoves.size() - 1);
        lastMove.undoMove();
        if (lastMove.didDestroyChessman())
    ...
    public void printPlayerMoves() {
        for (game.Move move : allGameMoves) {
    ...
    public void printPlayerKilledChessmen() {
        for (game.Chessman killedChessman : killedChessmen) {
    ...
    public void printBoard() {
        ...
        for (int i = 7; i >= 0; i--) {
            game.BoardUnitSquare[] boardRow = board[i];
            StringBuilder row = new StringBuilder();
            for (game.BoardUnitSquare square : boardRow) {
    ...
    private void endGame(int player1Score, int player2Score) {
        ...
        menus.Menu.setActiveMenu(new menus.UserMenu(whitePlayer.getUser()));
    }
    ...
    private void putInitialChessmen() {
        //Pawns
        for (int i = 0; i < 8; i++) {
            board[1][i].setCurrentChessman(new game.Chessman(whitePlayer, 'P', board[1][i]));
            board[6][i].setCurrentChessman(new game.Chessman(blackPlayer, 'P', board[6][i]));
        }
        // White back
        board[0][0].setCurrentChessman(new game.Chessman(whitePlayer, 'R', board[0][0]));
        board[0][7].setCurrentChessman(new game.Chessman(whitePlayer, 'R', board[0][7]));
        board[0][1].setCurrentChessman(new game.Chessman(whitePlayer, 'N', board[0][1]));
        board[0][6].setCurrentChessman(new game.Chessman(whitePlayer, 'N', board[0][6]));
        board[0][2].setCurrentChessman(new game.Chessman(whitePlayer, 'B', board[0][2]));
        board[0][5].setCurrentChessman(new game.Chessman(whitePlayer, 'B', board[0][5]));
        board[0][3].setCurrentChessman(new game.Chessman(whitePlayer, 'Q', board[0][3]));
        board[0][4].setCurrentChessman(new game.Chessman(whitePlayer, 'K', board[0][4]));
        // Black back
        board[7][0].setCurrentChessman(new game.Chessman(blackPlayer, 'R', board[7][0]));
        board[7][7].setCurrentChessman(new game.Chessman(blackPlayer, 'R', board[7][7]));
        board[7][1].setCurrentChessman(new game.Chessman(blackPlayer, 'N', board[7][1]));
        board[7][6].setCurrentChessman(new game.Chessman(blackPlayer, 'N', board[7][6]));
        board[7][2].setCurrentChessman(new game.Chessman(blackPlayer, 'B', board[7][2]));
        board[7][5].setCurrentChessman(new game.Chessman(blackPlayer, 'B', board[7][5]));
        board[7][3].setCurrentChessman(new game.Chessman(blackPlayer, 'Q', board[7][3]));
        board[7][4].setCurrentChessman(new game.Chessman(blackPlayer, 'K', board[7][4]));
    }
}
